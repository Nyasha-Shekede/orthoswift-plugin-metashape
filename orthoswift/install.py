...
def install(python: str | None = None, skip_dependencies: bool = False) -> dict:
    if not ENTRY.is_file() or not (HERE / "runner.py").is_file():
        raise RuntimeError(
            "Incomplete installation bundle. Ensure all plugin files are extracted."
        )
    ...
    if version < (3, 10) or version > (3, 12):
        raise RuntimeError(
            f"OrthoSWIFT requires Python 3.10-3.12; selected {version[0]}.{version[1]}"
        )
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description="Install OrthoSWIFT for the current Metashape user"
    )
    parser.add_argument(
        "--python",
        help="Python 3.10-3.12 executable used to create the isolated environment",
    )
    parser.add_argument(
        "--skip-dependencies",
        action="store_true",
        help="Use the selected Python directly; advanced/offline use",
    )
    parser.add_argument("--uninstall", action="store_true")
    args = parser.parse_args(argv)
    if args.uninstall:
        uninstall()
        print("Removed OrthoSWIFT Metashape startup loader.")
        return 0
    try:
        ...
        print(
            "Check that Python 3.10-3.12 and its venv support are installed, then retry.",
            file=sys.stderr,
        )
        return 1
    print(json.dumps(result, indent=2))
    print("Restart Metashape. OrthoSWIFT will appear in the menu.")
    return 0
